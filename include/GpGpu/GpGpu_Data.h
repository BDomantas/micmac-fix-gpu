...
protected:
	...
	///
	/// \brief abDealloc D�sallocation de la m�moire
	/// \return true si la d�sallocation est reussie
	///
    virtual bool    abDealloc(){ return false;} // TODO pour le rendre completement virtuelle il faut reimplementer les destructeurs...

	///
	/// \brief abMalloc Allocation de la m�moire
	/// \return
	///
    virtual bool    abMalloc(){ return false;}

	///
	/// \brief Sizeof retourn le taille de la structure en m�moire
	/// \return
	///
...
public:
	...
	///
	/// \brief ReallocIfDim Reallocation m�moire si le dimension est sup�rieure � la dimension actuelle
...
public:
	...
	///
	/// \brief Realloc Reallocation de la m�moire de taille dim
	...
	///
	/// \brief ReallocIf Reallocation si la nouvelle taille est sup�rieure � l'actuelle
	...
	///
	/// \brief ReallocIfDim Reallocation si la nouvelle taille est sup�rieure � l'actuelle
	/// \param dim taille de dimension 2
	/// \param l taille sur la dimension Z
	/// \return
	///
	...
	///
	/// \brief operator []
	/// \param pt coordonn�es de la donn�es requeter
...
public:
	...
	/// \param pgLockMem Option de m�moire pagin�e
	/// \param alignMemory Option de m�moire align�e
	///
	...
	/// \brief pLData
	/// \param layer
	/// \return  le pointeur du calque point�e
	///
	...
	///
	/// \brief pgLockMem
	/// \return Option de m�moire pagin�e
	///
	...
	///
	/// \brief alignMemory
	/// \return Option de m�moire align�e
	///
...
/// \brief Structure 3d de donn�es instanci�es dans la m�moire globale vid�o
template <class T>
...
public:
	...
	/// \param dim	Dimension 2D de la structure de donn�es
	/// \param l	Taille de la structure dans la dimansion 3 (Z)
	/// \param name Nommer la sctructure pour le d�bogage
	///
	...
	/// \param dim	Dimension totale de la structure de donn�es
	/// \param name Nommer la sctructure pour le d�bogage
	...
	///
	/// \brief CopyDevicetoHost Copier le contenue de la m�moire globale de la structure vers l'hote
	/// \param hostData pointeur hote de destination
	/// \return vrais si l'op�ration a r�ussi
	///
...
public:
    ...
    /// Phase C: public so SData2Correl can build cudaTextureObject_t handles.
...
public:
	...
	///
	/// \brief SetNameImage Nommer l'image pour le d�bogage
...
/// Structure de donn�es unifi�es
/// Cette structure de donn�es est constitu�es d'un espace m�moire sur l'hote et un espace identique sur le device
struct CuUnifiedData3D
{
	///
	/// \brief deviceData les donn�es sur le device
	///
    CuDeviceData3D<T>   deviceData;
	///
	/// \brief hostData les donn�es sur l'hote
	///
    CuHostData3D<T>     hostData;

	///
	/// \brief Malloc Allocation de  la m�moire pour l'hote et le device
	/// \param dim Taille 2d des structures de donn�es
	/// \param l Taille en Z des structures de donn�es
	///
	...
	///
	/// \brief syncDevice
	/// Copier les donn�es de l'hote vers le device
    void syncDevice()
    {
	...
	///
	/// \brief syncHost
	/// Copier les donn�es du device vers l'hote
    void syncHost()
    {
	...
	///
	/// \brief ReallocIfDim Reallocation m�moire si la dimension totale est sup�rieur � la pr�c�dente
	/// \param dim Taille 2d des structures de donn�es
	/// \param l Taille en Z des structures de donn�es
	///
	...
	///
	/// \brief Reallocation m�moire si la dimension totale est sup�rieur � la pr�c�dente
	...
	///
	/// \brief Dealloc D�salocation de la m�moire des deux structures
	///
    void Dealloc()
    {
        deviceData. Dealloc();
	...
	///
	/// \brief pData
	/// \return le pointeur sur les donn�es du device
	///
    T* pData()
    {
	...
	///
	/// \brief SetName Nommer la structure pour le d�bogage
	/// \param name Nom
