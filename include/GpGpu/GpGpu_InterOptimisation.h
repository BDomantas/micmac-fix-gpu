...
/// Structure 1D des couts de cor�lation
struct sMatrixCellCost
{
	///
	/// \brief _CostInit1D
	/// les couts intrins�ques des cellules
	CuHostData3D<T>         _CostInit1D;
	///
	/// \brief _ptZ
	/// Coordonn�es Z de la nappe d'une position terrain
	...
	///
	/// \brief _pit
	/// Decallage pour acceder au d�but d'une  ligne
	...
	///
	/// \brief ReallocPt Reallocation de la m�moire conditionelle
	/// \param dim
	///
	...
	///
	/// \brief ReallocData Reallocation de la m�moire conditionelle
	///
    void                    ReallocData()
    {
	...
	/// \brief fillCostInit
	/// \param val
	/// Remplir la strucutre par la donn�e val
	...
	///
	/// \brief Dealloc D�salocation de la m�moire
	///
    void                    Dealloc()
    {
        _CostInit1D.Dealloc();
	...
	///
	/// \brief setDefCor D�finir la valeur par d�faut de corr�lation
	/// \param pt
	/// \param defCor
	///
	...
	/// \brief Pit
	/// \param pt
	/// \return le d�calage pour un point pt
	///
    uint                    Pit(uint2 pt)
    {
        return _pit[pt];
	...
	/// \brief PtZ
	/// \param pt
	/// \return Les coordonn�es Zmin et Zmax
	///
    short3                  PtZ(uint2 pt)
    {
        return _ptZ[pt];
	...
	/// \brief operator []
	/// \param pt
	/// \return le cout intrins�que du point
	///
    T*                      operator[](uint2 pt)
    {
...
public:
    ...
    ///
	/// \brief Data2Opt
	/// \return les donn�es device
    ///
    HOST_Data2Opti& HData2Opt(){ return _H_data2Opt;}
	///
	/// \brief DData2Opt
	/// \return Les donn�es hote
	///
	...
	///
	/// \brief Prepare Initialisation des param�tres
	/// \param x
	/// \param y
	/// \param penteMax
	/// \param NBDir
	...
	/// Structure de donn�es des couts forc�es
    CuHostData3D<uint>      _preFinalCost1D;

	///
	/// \brief _FinalDefCor
	/// Structure de donn�es des cellules de defCor
