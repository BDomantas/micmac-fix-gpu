        ...
        if (mCorrelAdHoc->TypeCAH().GPU_CorrelBasik().IsInit())
        {
            if (!IMmGg.TexturesAreLoaded())//		Mise en calque des images
            {
                ...
                if(IMmGg.NoMasked)
                {
                    ...
                    // Pour chaque image nous copions les valeurs dans une structure preparatoire pour les envoy�s au GPU
                    for (int aKIm=0 ; aKIm<mNbIm ; aKIm++)
                    {
            ...
            //
            // G�n�ration de volumes, o� le calcul est n�cessaire.
...
///
/// \brief cAppliMICMAC::Tabul_Projection Pr�-calcul des projections des points terrains dans chaque images
/// \param Z        Z initiale
/// \param interZ   interval de Z � pr�-calcul�
/// \param idBuf    id buffer de pr�-calcul
///
	void cAppliMICMAC::Tabul_Projection(short Z, ushort &interZ, ushort idBuf)
    {
		...
		const uint2	dimSTabProj	=	iDivUp(dimTabProj,sample)+1;		// Dimension de la zone terrain sous-echantillon�
		...
		float2* buf_proj		= pTabProj ;//+ (rZ  + aKIm )* sizSTabProj;	// Buffer des projections pre-calcul�es
		OMP_NT1
		for (short anZ = Z; anZ < (Z + interZ); ++anZ)
		{
			...
			for (ushort aKIm = 0 ; aKIm < mNbIm ; ++aKIm)        // Mise en calque des projections pour chaque image
            {
				...
				int2  pTer;                                 // Debut de la zone de pr�-calcul
				Pt2dr pTerDequan;
    ...
    void cAppliMICMAC::Tabul_Images(int Z, uint &interZ, ushort idBuf)
    {
        ...
        uint	sizSTabProj	= size(dimTabProj);					// Taille de la zone terrain sous-echantillon�
        ...
        for (int anZ = Z; anZ < (int)(Z + interZ); anZ++)
        {
            ...
            for (int aKIm = 0 ; aKIm < mNbIm ; aKIm++ )                     // Mise en calque des projections pour chaque image
            {
                ...
                float* buf_ValImages     = pBufVimg + (rZ  + aKIm )* sizSTabProj;// Buffer des projections pre-calcul�es

                cGPU_LoadedImGeom&	aGLI	= *(mVLI[aKIm]);                // Obtention de l'image aKIm
                const cGeomImage*	aGeom	= aGLI.Geom();

                int2 pTer       = zone.pt0;                                 // Debut de la zone de pr�-calcul
		...
		for (int anY = zone.pt0.y ; anY < zone.pt1.y; anY++)
		{
			...
			for (int anX = zone.pt0.x ; anX <  zone.pt1.x ; anX++,tabCost++)
			{
				...
				for (int anZ = anZ0;  anZ < anZ1 ; anZ++,tCost+=rSiTer)
                {
                    ...
                    // TODO WARNING les couts init sont stock�s dans un ushort mais des couts semblent sup � ushortmax!!!!
...
unsigned long _gpgpu_idle = 0;
            ...
            {
                bool _did = false; // RUNPOD_GPGPU_DIAG
